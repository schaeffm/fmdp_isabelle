...
lemma \<T>_in_S: \<open>\<T> p s \<in> space (subprob_algebra S)\<close>
  by (simp add: \<T>_def)

lemma count_space_prod[simp]: \<open>count_space (UNIV :: (_ :: countable) set) \<Otimes>\<^sub>M count_space (UNIV :: (_ :: countable) set) = count_space UNIV\<close>
  using pair_measure_countable[of UNIV UNIV]
  by fastforce

lemma \<open>sets (measure_pmf (p [] s)) = sets (count_space UNIV)\<close>
  by auto

lemma AE_\<T>: 
  assumes f[measurable]: "Measurable.pred (stream_space (count_space UNIV)) P"
  shows "(AE t in \<T> p s. P t) \<longleftrightarrow> 
    (AE a in p [] s. AE s' in K (s,a). AE t' in \<T> (\<pi>_Suc p (s,a)) s'. P ((s,a)##t'))"
  apply (subst \<T>_eq)
  apply (subst Giry_Monad.AE_bind[of _ _ S])
  subgoal by (auto intro!: subprob_space.bind_in_space prob_space_imp_subprob_space)
  subgoal by measurable
  subgoal
    apply (rule AE_cong)
    apply (subst Giry_Monad.AE_bind[of _ _ S])
    by (auto intro!: subprob_space.bind_in_space prob_space_imp_subprob_space simp: 
        prob_space_imp_subprob_space  Giry_Monad.AE_bind[of _ _ S] Giry_Monad.AE_return space_stream_space)
  done
