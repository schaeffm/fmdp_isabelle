...
locale MDP_reward_summable = MDP_reward A K r l
  for
    A and 
    K :: "'s ::countable \<times> 'a ::countable \<Rightarrow> 's pmf" and
    r l +
  assumes
    rewards_summable: "\<And>p. p \<in> \<Pi>\<^sub>H\<^sub>R \<Longrightarrow> summable (\<lambda>i. l^i * measure_pmf.expectation (Pn' p s i) r)"
begin
...
lemma contraction_\<L>_dist: \<open>(1 - l) * dist v \<nu>\<^sub>b_opt \<le> dist v (\<L>\<^sub>b v)\<close>
  using contraction_dist[OF contraction_\<L>, of v]
  by auto
...
lemma someI_bex: \<open>\<exists>a\<in>X. P a \<Longrightarrow> P (SOME x. x \<in> X \<and> P x) \<and> (SOME x. x \<in> X \<and> P x) \<in> X\<close>
  using someI2_bex[of X P \<open>\<lambda>x. P x \<and> x \<in> X\<close>]
  by auto
...
shows "\<exists>d \<in> D\<^sub>D. L (mk_dec_det d) (v :: _ \<Rightarrow>\<^sub>b _) = \<L>\<^sub>b v"
  ...
  using ex_opt_act someI_bex[OF ex_opt_act] assms L_eq_L\<^sub>a
  by (auto intro!: exI[of _ \<open>\<lambda>s. SOME a. a \<in> A s \<and> L\<^sub>a a v s = \<L>\<^sub>b v s\<close>])
