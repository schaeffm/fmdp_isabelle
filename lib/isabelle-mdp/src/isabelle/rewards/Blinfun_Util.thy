...
lemma tendsto_blinfun_apply': "u \<longlonglongrightarrow> L  \<Longrightarrow> (\<lambda>n. blinfun_apply (X :: 'd::real_normed_vector \<Rightarrow>\<^sub>L 'b::real_normed_vector) (u n)) \<longlonglongrightarrow> X L"
  apply (rule Limits.bounded_linear.tendsto[of \<open>blinfun_apply X\<close>])
