...
lemma sum_apply_bfun':
  fixes f :: "'b \<Rightarrow> 'c \<Rightarrow>\<^sub>b real" 
  shows "(\<Sum>i \<in> B. f i) x = (\<Sum>i \<in> B. apply_bfun (f i) x)"
  apply (cases \<open>finite B\<close>)
  by (induction B rule: finite_induct) auto
...
find_theorems "\<bar>\<Squnion>_\<in> _. _\<bar>"
end
