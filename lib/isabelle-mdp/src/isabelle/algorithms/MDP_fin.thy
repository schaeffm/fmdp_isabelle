...
print_locale MDP_act
locale MDP_on = MDP_act_disc arb_act A K r
  for
    A and
...
proof -
  ...
  thus ?thesis
    unfolding \<L>\<^sub>b_eq_L\<^sub>a_max L\<^sub>a_int by auto
qed
...
locale MDP_nat_disc = MDP_nat r l arb_act A K states + MDP_act  A K r l arb_act
